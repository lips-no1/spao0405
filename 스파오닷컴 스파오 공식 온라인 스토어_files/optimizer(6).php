...
CAPP_ASYNC_METHODS.AppCommon = {
        ...
        return new Promise(function (resolve, reject) {
            ...
            new Promise(function (res, rej) {
                ...
                EC$.post('/exec/front/order/reserve/', aInfo, function(data) {
                    ...
                    aPrecreateOrderResult['hmac'] = data['hmac'];
                    aPrecreateOrderResult['response_time'] = data['response_time'];
    ...
    getOrderHistoryItemList: function(history_start_date, history_end_date, order_status, page, count, order_id)
    {
        count = count === null ? iOrderHistoryLimit : 15;
        return new Promise(function (resolve, reject) {
            if (history_start_date || order_id || count != 15) { //기본조회값과 조건이 다른경우 새롭게 갱신
                CAPP_ASYNC_METHODS.OrderHistoryItemList.getAsyncData(history_start_date, history_end_date, order_status, page, count, order_id)
                    .then(function(oData) {
                        resolve(oData);
                    }).catch(function(data) {
                        reject(data);
                ...
                if (oData) {
                    var oOrderHistoryItemList = EC_EXTERNAL_UTIL_APP_SPECINTERFACE.oOrderHistoryItemList;
                    var aOrderHistoryItemList = [];

                    for (var iKey in oData) {
                        aOrderHistoryItemList.push(CAPP_ASYNC_METHODS.AppCommon.setSpecData(oOrderHistoryItemList, oData[iKey]));
                    }
